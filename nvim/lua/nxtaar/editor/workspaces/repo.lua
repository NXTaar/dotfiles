...
function Repo:add()
    local answer = vim.fn.confirm(
        self.name .. ' - Add this repo to the workspace?',
        '&Yes\n&No',
        2,
        'Question'
    )
    ...
    vim.notify(
        'Repo ' .. self.name .. ' has been added to the workspace',
        vim.log.levels.INFO
    )
end
...
function Repo:remove()
    local answer = vim.fn.confirm(
        self.name .. ' - Remove this repo from the workspace?',
        '&Yes\n&No',
        2,
        'Question'
    )
    ...
    vim.notify(
        'Repo ' .. self.name .. 'has been removed from the workspace',
        vim.log.levels.INFO
    )
end
